...
export function OverviewTab({ stats, accentColor, archiveItemsCount }: OverviewTabProps) {
  const { kpis, goalProgress, sparklines } = stats;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <StatCard
          ...
          sparklineData={sparklines.readingTime}
        />
        <StatCard
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-medium opacity-80 flex items-center gap-2">
            <CalendarIcon />
            Goal Progress
          </h3>
          <button
        ...
        {hasGoals ? (
          ...
          <div 
            className="flex flex-col items-center justify-center py-8 bg-reader-text/5 rounded-lg cursor-pointer hover:bg-reader-text/10 transition-colors"
            onClick={() => setShowGoalEditor(true)}
          >
            <svg className="w-8 h-8 opacity-30 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
            </svg>
            <p className="text-sm opacity-50">Set reading goals to track your progress</p>
          </div>
        )}
      </div>
      ...
      {showGoalEditor && (
        <GoalEditor
          currentGoals={stats.stats?.goals ?? null}
